param(
    [int[]]$Seeds = @(5),
    [string]$ResultRoot = "results\epoch200_compare",
    [int]$Epochs = 200,
    [int]$EvalStep = 1024,
    [int]$BatchSize = 32,
    [int]$Mu = 7
...
foreach ($Seed in $Seeds) {
    foreach ($Experiment in $Experiments) {
        ...
        "[$(Get-Date -Format s)] Starting seed=$Seed $($Experiment.Name), epochs=$Epochs, total_steps=$TotalSteps, batch=$BatchSize, mu=$Mu" |
