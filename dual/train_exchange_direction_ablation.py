...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Dual FixMatch exchange-direction ablation')
    ...
    parser.add_argument('--dual-train-mode', default='minor_weighted_balanced',
                        choices=['minor_weighted_balanced'],
                        help='Fixed mode: only the minor model uses balanced sampling and weighted CE.')
    parser.add_argument('--exchange-mode', default='bidirectional',
                        choices=['none', 'major_to_minor',
                                 'minor_to_major', 'bidirectional'],
                        help='Controls which pseudo-labels are exchanged between major and minor models.')
...
def main():
    ...
    logger.info('  Exchange mode = %s', args.exchange_mode)
...
def train_dual_bias(args, labeled_trainloader, minor_labeled_trainloader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if args.exchange_mode == 'none':
                major_targets_u, major_mask_u = targets_major, mask_major
                minor_targets_u, minor_mask_u = targets_minor, mask_minor
            elif args.exchange_mode == 'major_to_minor':
                major_targets_u, major_mask_u = targets_major, mask_major
                minor_targets_u, minor_mask_u = targets_major, mask_major
            elif args.exchange_mode == 'minor_to_major':
                major_targets_u, major_mask_u = targets_minor, mask_minor
                minor_targets_u, minor_mask_u = targets_minor, mask_minor
            else:
                major_targets_u, major_mask_u = targets_minor, mask_minor
                minor_targets_u, minor_mask_u = targets_major, mask_major

            Lu_major = (F.cross_entropy(logits_u_s_major, major_targets_u,
                                        reduction='none') * major_mask_u).mean()
            Lu_minor = (F.cross_entropy(logits_u_s_minor, minor_targets_u,
                                        reduction='none') * minor_mask_u).mean()
        ...
        write_train_metrics(args, epoch, {
            'mode': args.dual_train_mode + '_' + args.exchange_mode,
