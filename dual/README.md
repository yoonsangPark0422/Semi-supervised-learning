...
This folder contains the cleaned dual-model training code used for the
...
- `train.py`: main dual training entry point.
- `run_dual.ps1`: single CIFAR-10-LT dual run.
- `run_dual_mode_compare.ps1`: runs the three sampler / weighted-CE ablation
  modes under the same setting.
- `models/`, `dataset/`, `utils/`: model, dataset, and utility code copied for
