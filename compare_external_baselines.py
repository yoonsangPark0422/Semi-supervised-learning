...
def _early_dual_dir_arg():
    ...
    parser.add_argument('--dual-dir', default='')
    args, _ = parser.parse_known_args()
    return args.dual_dir


def _has_training_helpers(path):
    train_py = path / 'train.py'
    if not train_py.exists():
        return False
    try:
        return 'PseudoLabelStats' in train_py.read_text(encoding='utf-8')
    except UnicodeDecodeError:
        return 'PseudoLabelStats' in train_py.read_text(errors='ignore')


def find_dual_dir():
    explicit = _early_dual_dir_arg() or os.environ.get('DUAL_DIR', '')
    if explicit:
        path = Path(explicit).expanduser()
        if not path.is_absolute():
            path = ROOT / path
        if _has_training_helpers(path):
            return path.resolve()
        raise RuntimeError(
            '--dual-dir must point to the folder that contains train.py with '
    ...
    candidates.extend(
        ...
        if path.is_dir() and path.name.lower().startswith('dual'))
    ...
    raise RuntimeError(
        'Could not find the dual training folder. Put the dual folder next to '
        'compare_external_baselines.py, or run with '
        '--dual-dir /path/to/dual-folder. Searched: %s' % searched)


DUAL_DIR = find_dual_dir()
...
def make_args(cli, method, out_dir):
    ...
    args.log_interval = max(1, cli.log_interval)
    args.method = method
    args.num_classes = 10
    args.model_depth = 28
    args.model_width = 2
...
def train_baseline(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        iterator = tqdm(
            range(args.eval_step),
            disable=args.no_progress,
            dynamic_ncols=True,
            desc='%s epoch %s/%s' % (args.method, epoch + 1, args.epochs))
        for step in iterator:
            ...
            if global_step % args.log_interval == 0:
                args.writer.add_scalar('train_step/loss', loss.item(), global_step)
                args.writer.add_scalar('train_step/loss_x', Lx.item(), global_step)
                args.writer.add_scalar('train_step/loss_u', Lu.item(), global_step)
                args.writer.add_scalar('train_step/mask', mask.mean().item(),
                                       global_step)
                args.writer.add_scalar(
                    'train_step/lr', scheduler.get_last_lr()[0], global_step)
                args.writer.flush()
            if not args.no_progress and (
                    step % args.log_interval == 0 or step + 1 == args.eval_step):
                iterator.set_postfix({
                    'loss': '%.4f' % losses.avg,
                    'Lx': '%.4f' % losses_x.avg,
                    'Lu': '%.4f' % losses_u.avg,
                    'mask': '%.3f' % mask_probs.avg,
                    'lr': '%.5f' % scheduler.get_last_lr()[0],
                })
        ...
        args.writer.add_scalar('train/loss_x', losses_x.avg, epoch)
        args.writer.add_scalar('train/loss_u', losses_u.avg, epoch)
...
def parse_args():
    ...
    parser.add_argument('--methods', default='fixmatch,crest,daso,abc,proposed')
    ...
    parser.add_argument('--out', default=str(ROOT / 'baseline_compare_3day'))
    parser.add_argument('--dual-dir', default='',
                        help='Path to the proposed-model training folder that contains train.py.')
    parser.add_argument('--log-interval', type=int, default=20,
                        help='Update live tqdm/TensorBoard train-step logs every N steps.')
    parser.add_argument('--no-progress', action='store_true',
                        help='Disable live tqdm progress bars.')
