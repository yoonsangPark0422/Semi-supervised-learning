...
def target_group_mask(targets, class_counts, args, group):
    ...
    if group == 'head':
        return head_mask[targets]
    if group == 'tail':
        return tail_mask[targets]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Head-Tail Specialist Dual FixMatch Training')
    ...
    parser.add_argument('--major-cross-skip-minor-conf-threshold', default=-1.0, type=float,
                        help='Disabled by default in this ablation. Set >=0 to skip Major->Minor cross when Minor confidence is high.')
    parser.add_argument('--minor-balanced-sampler-alpha', default=1.2, type=float,
                        help='Exponent for minor balanced sampler weights: weight = 1 / class_count ** alpha. alpha > 1 biases more toward tail classes.')
    parser.add_argument('--T', default=1, type=float)
    parser.add_argument('--threshold', default=0.95, type=float)
    parser.add_argument('--dual-train-mode', default='dual_sampler_weighted_ce',
                        choices=['dual', 'dual_sampler', 'dual_weighted_ce',
    ...
    args.use_minor_weighted_ce = args.dual_train_mode in [
        'dual_weighted_ce', 'dual_sampler_weighted_ce']
    return args
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if global_step < args.pseudo_warmup:
                ...
                mask_minor_self = torch.zeros_like(mask_minor_self)
